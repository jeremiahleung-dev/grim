...
struct SocialAgent {
    func run(birthdays: [String], completion: @escaping (CandidateSuggestion?) -> Void) {
        guard !birthdays.isEmpty else { completion(nil); return }

        let system = "You are the social advisor in a life-awareness app. Given upcoming birthdays, identify who the user should reach out to and why right now."

        let user = """
        Upcoming birthdays: \(birthdays.joined(separator: "; "))

        Respond only with JSON: {"suggestion":"...","urgency":N}
        The suggestion must be max 15 words, specific (name the person). Urgency is 1–10. No explanation.
        """

        callHaiku(system: system, user: user) { text in
            completion(parse(text: text, domain: "social"))
        }
    }
