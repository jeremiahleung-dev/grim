...
struct EnvironmentAgent {
    func run(weather: String?, completion: @escaping (CandidateSuggestion?) -> Void) {
        guard let weather, !weather.isEmpty else { completion(nil); return }

        let system = "You are the environment advisor in a life-awareness app. Given current weather conditions, identify what the user should do or be aware of today."

        let user = """
        Current conditions: \(weather)

        Respond only with JSON: {"suggestion":"...","urgency":N}
        The suggestion must be max 15 words, actionable. Urgency is 1–10. No explanation.
        """

        callHaiku(system: system, user: user) { text in
            completion(parse(text: text, domain: "environment"))
        }
    }
