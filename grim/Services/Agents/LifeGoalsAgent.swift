...
struct LifeGoalsAgent {
    func run(goals: [String], completion: @escaping (CandidateSuggestion?) -> Void) {
        guard !goals.isEmpty else { completion(nil); return }

        let system = "You are the aspirations advisor in a life-awareness app. Given the user's life goals, identify one small, concrete action they can take today to move toward any of them."

        let user = """
        The user's life goals: \(goals.joined(separator: "; "))
        ...
        callHaiku(system: system, user: user) { text in
            completion(parse(text: text, domain: "goals"))
        }
    }
